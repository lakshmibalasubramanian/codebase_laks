...
base_path <- "/media/lab-admin/VS_lab_disk/Lakshmi_working_data/SurfaceArea_analysis_new_Jun17/"
  ...
  for (j in 1:length(file_timepoint)){
    ...
    loca_inf[[j]] <- 
      cbind(
        ...
        lapply(dat_saBC_inf, function(x) x[[3]] %>% as.numeric %>% sum()) %>% as.matrix(),
        lapply(dat_saBC_inf, function(x) x[[5]] %>% as.numeric %>% sum()) %>% as.matrix(),
        lapply(dat_saBC_int_intensity_inf, function(x) x[[6]] %>% as.numeric() %>% sum()) %>% as.matrix()
        ) %>% as.data.frame()
    ...
    loca_uninf[[j]] <- 
      cbind(
            ...
            lapply(dat_uninf_saBC, function(x) x[[3]] %>% as.numeric %>% sum())  %>% as.matrix(), 
            lapply(dat_uninf_saBC, function(x) x[[5]] %>% as.numeric %>% sum()) %>% as.matrix(),
...
write.csv(total_inf, file = "/media/lab-admin/VS_lab_disk/Lakshmi_working_data/SurfaceArea_analysis_new_Jun17/Analysis_SA_vol_24_48/infected_data.csv")
write.csv(total_pvm, file = "/media/lab-admin/VS_lab_disk/Lakshmi_working_data/SurfaceArea_analysis_new_Jun17/Analysis_SA_vol_24_48/pvm_data.csv")
write.csv(total_uninf, file = "/media/lab-admin/VS_lab_disk/Lakshmi_working_data/SurfaceArea_analysis_new_Jun17/Analysis_SA_vol_24_48/uninfected_data.csv")

write.csv(total_all, file = "/media/lab-admin/VS_lab_disk/Lakshmi_working_data/SurfaceArea_analysis_new_Jun17/Analysis_SA_vol_24_48/master_data.csv")
View(total_all)
